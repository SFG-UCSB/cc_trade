...
for(i in 1:length(rcps)){
  ...
  # Setup figure
  figname <- paste0("Fig2_country_scatterplot_", tolower(rcps[i]), ".png")
  ...
  for(j in 1:length(scenarios)){
    ...
    # Plot data
    plot(h_prop ~ b_prop, sdata, bty="n", las=1,
         xlab="Difference in biomass", ylab="Difference in harvest",
         main=scenarios[j], bg=sdata$p_prop_col, pch=21, cex=1.5, cex.axis=0.9,
