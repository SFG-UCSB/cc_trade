...
format_aquamaps <- function(file, filedir){
  ...
  pdata <- data_orig %>% 
    ...
    rename(genus=col1, species=col2, lat_dd=col3, long_dd=col4, cell_id=col5, prob=col6) %>% 
  ...
  edata <- data_orig %>% 
    ...
    rename(genus=col1, species=col2, lat_dd=col3, long_dd=col4, cell_id=col5, 
  ...
  qdata <- data_orig %>%
    slice(qdata1:qdata2) %>% 
    dplyr::select(paste0("col", 1:6)) %>% 
    rename(variable=col1, used=col2, min=col3, q10=col4, q90=col5, max=col6) %>% 
  ...
  data <- list(pdata=pdata, pdata_mat=pdata_mat, edata=edata, qdata=qdata)
  return(data)
